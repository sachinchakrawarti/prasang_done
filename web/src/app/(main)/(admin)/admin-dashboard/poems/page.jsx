// src/app/(main)/(admin)/admin-dashboard/poems/page.jsx
"use client";
...
import { fetchPoems, deletePoem, updatePoem } from "@/services/adminService";
...
export default function AdminPoemsPage() {
  ...
  const loadPoems = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        const response = await fetchPoems(params);
  ...
  // Load filter options (poets, categories)
  ...
  const handleBulkDelete = async (ids) => {
    if (
      !confirm(
        `Are you sure you want to delete ${ids.length} poems? This action cannot be undone.`,
      )
    ) {
      return;
    }
    ...
    setIsDeleting(true);
    try {
      await Promise.all(ids.map((id) => deletePoem(id)));
      // Remove from list
      setPoems(poems.filter((p) => !ids.includes(p.id)));
      setSelectedIds([]);
      // Update total count
      setPagination((prev) => ({
        ...prev,
      ...
      console.error("Failed to delete poems:", err);
      alert("Failed to delete poems: " + err.message);
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await updatePoem(id, { status: newStatus });
      // Update in list
      setPoems(
        poems.map((p) => (p.id === id ? { ...p, status: newStatus } : p)),
      );
    } catch (err) {
  ...
  const handleSelectAll = (checked) => {
    setSelectedIds(checked ? poems.map((p) => p.id) : []);
  };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {pagination.total} {t?.("totalPoems") || "total poems"}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/admin-dashboard/poems/new"
      ...
      {!isLoading && poems.length === 0 && !error && (
        <div
        ...
        >
          ...
          >
            <FaPlus size={14} />
            {t?.("addFirstPoem") || "Add your first poem"}
          </Link>
        </div>
      )}
    </div>
