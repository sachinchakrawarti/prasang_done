...
const samplePoems = [
  ...
  },
  {
    ...
    content: "मैंने देखा एक सपना, जिसमें थी बहार, फूल खिले थे हर किनारे, थी खुशियों की सौगात...",
    type: "गीत",
    language: "hi",
    likes: 567,
    views: 2345,
...
export default function PoemsPage() {
  ...
  const types = ["all", ...new Set(poems.map(p => p.type))];
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 space-y-4">
          ...
          {/* Filters Panel */}
          {showFilters && (
            <div className={`p-4 rounded-lg border ${borderColor} bg-white dark:bg-gray-800 grid grid-cols-1 sm:grid-cols-3 gap-4`}>
              {/* Type Filter */}
              <div>
                ...
                >
                  {languages.map((lang) => (
                    <option key={lang} value={lang}>
                      {lang === "all" ? (t("allLanguages") || "All Languages") : lang}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <option value="popular">{t("mostLiked") || "Most Liked"}</option>
        ...
        <div className="mt-8 text-center text-sm text-gray-500 dark:text-gray-400">
          {t("showing") || "Showing"} {filteredPoems.length} {t("poems") || "poems"}
        </div>
      </div>
    </div>
  );
...
function PoemCard({ poem, lang, textColor, gradient, borderColor, hoverBg, themeName }) {
  const { t } = useTranslation();
  
  return (
    <Link
      href={`/${lang}/poems/${poem.id}`}
...
function PoemListItem({ poem, lang, textColor, gradient, borderColor, hoverBg, themeName }) {
  const { t } = useTranslation();
