// src/app/(main)/(admin)/admin-dashboard/categories/components/CategoryTable.jsx
"use client";
...
export default function CategoryTable({
  ...
  // Toggle category expansion (for nested view)
  ...
  const isAllSelected =
    categories.length > 0 &&
    categories.every((cat) => selectedIds.includes(cat.id));
  ...
  const renderCategoryRow = (category, level = 0) => {
    ...
    const isExpandedCategory = expandedCategories[category.id];

    return (
      <tbody key={category.id}>
        {/* Main Row */}
        <tr
          className={`${hoverBg} transition-colors ${selectedIds.includes(category.id) ? "bg-amber-50 dark:bg-amber-900/10" : ""}`}
        >
          ...
          <td className="px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              >
                {category.icon || <FaFolder size={12} />}
              </div>
          ...
          <td className="px-4 py-3 hidden sm:table-cell">
            <span
              className={`px-2 py-1 text-xs rounded-full ${getStatusBadge(category.status)}`}
            >
        ...
        {hasChildren && isExpandedCategory && (
          <tr>
            <td colSpan="6" className="px-4 py-0">
              <div className="pl-8 border-l-2 border-gray-200 dark:border-gray-700 ml-8">
          ...
          {selectedIds.length > 0 && (
            <>
              <button
                onClick={() => {
                  if (
                    confirm(`Delete ${selectedIds.length} selected categories?`)
                  ) {
              ...
              <button
                onClick={() => {
                  if (
                    confirm(
                      `Activate ${selectedIds.length} selected categories?`,
                    )
                  ) {
